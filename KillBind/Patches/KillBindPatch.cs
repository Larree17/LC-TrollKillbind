...
namespace KillBind.Patches
{
    ...
    public class KillBindPatch
    {
        ...
        public static void OnPressKill(PlayerControllerB __instance)
        {
            ...
            {
                if (BasePlugin.DeathCause.Value > Enum.GetValues(typeof(CauseOfDeath)).Length || BasePlugin.DeathCause.Value < 0) { BasePlugin.DeathCause.Value = (int)BasePlugin.DeathCause.DefaultValue; BasePlugin.mls.LogInfo("Your config for HeadType is invalid, defaulting to value 0"); } //If your choice is invalid, set to default (unknown death cause)
                if (BasePlugin.HeadType.Value > 3 || BasePlugin.HeadType.Value < 0) { BasePlugin.HeadType.Value = (int)BasePlugin.HeadType.DefaultValue; BasePlugin.mls.LogInfo("Your config for HeadType is invalid, defaulting to value 1"); } //If your choice is invalid, set to default (explode head)
